...
describe('Simulação de Nota de Ajuste Real (MGECOM)', () => {
  ...
  it('Deve criar nota real de ENTRADA (MGECOM 221) e CONFIRMAR', async () => {
    ...
    const divFinal = await prisma.divergencia.findFirst({
  ...
  it('Deve criar nota real de SAÍDA (MGECOM 1121) e CONFIRMAR', async () => {
    ...
    try {
      ...
      const hasCost = await sankhyaClient.getReplacementCost(
        codprod,
        codemp,
        codlocal,
      );
      if (!hasCost) {
        console.log(
          '⚠️ Sem custo na TGFCUS. Inserindo custo dummy via CRUD...',
        );
      ...
      const precoTab = await sankhyaClient.executeQuery(`
                SELECT 1 FROM TGFEXC WHERE CODPROD = ${codprod} AND NUTAB IN (SELECT NUTAB FROM TGFTAB WHERE DTVIGOR <= SYSDATE)
            `);

      if (!precoTab || precoTab.length === 0) {
        console.log(
          '⚠️ Sem preço de tabela (TGFEXC). Inserindo preço dummy via CRUD...',
        );
        // Pegar primeira tabela ativa
        let nutab = 0;
        const tabAtiva = await sankhyaClient.executeQuery(
          'SELECT MIN(NUTAB) as NUTAB FROM TGFTAB WHERE DTVIGOR <= SYSDATE',
        );
      ...
      console.warn(
        'Falha ao garantir custo/preço (pode falhar se usuário não tiver permissão de INSERT direto):',
        e,
      );
    }
  ...
  it('Deve tentar criar nota real usando o módulo MGECOM', async () => {
    ...
    await prisma.snapshotEstoque.upsert({
      where: {
        dataRef_codemp_codlocal_codprod: { dataRef, codemp, codlocal, codprod },
      },
    ...
    const contagem = await prisma.contagem.create({
      data: {
        ...
        tsInicio: new Date(),
        tsFim: new Date(),
        snapshotId: (await prisma.snapshotEstoque.findFirst({
          where: { codprod },
        }))!.id,
        divergencia: 2,
