...
describe('Inteligência de Divergência (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    const login1Res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ login: op1Login, senha: '123456' });
    ...
    const login2Res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ login: op2Login, senha: '123456' });
    ...
    const loginSupRes = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ login: supLogin, senha: '123456' });
  ...
  describe('Recontagem Automática', () => {
    it('1ª contagem com divergência > 5% → dispara recontagem automática', async () => {
      ...
      console.log(
        '📊 Resultado 1ª contagem:',
        JSON.stringify(registrarRes.body),
      );
  ...
  describe('Contagem OK', () => {
    it('Contagem sem divergência → item finalizado automaticamente', async () => {
      ...
      await prisma.divergencia.deleteMany({
        where: { contagem: { codprod: codprodOk } },
      });
  ...
  describe('Regra "Não Achei"', () => {
    it('Operador 1 marca "Não Achei" → item volta pra fila', async () => {
      ...
      const fila = await prisma.filaContagem.findFirst({
        where: { codprod: CODPROD_NAO_ACHEI },
      });
  ...
  describe('Ação FINALIZAR_ANALISE', () => {
    it('Supervisor finaliza análise → item vai pro final da fila', async () => {
      ...
      const tratarRes = await request(app.getHttpServer())
        ...
        .send({
          acao: 'FINALIZAR_ANALISE',
          observacao: 'Produto em investigação',
        })
        .expect(201);
