...
export class SankhyaClient {
  ...
  constructor(private configService: ConfigService) {
    const baseURL =
      this.configService.get<string>('SANKHYA_API_URL') ||
  ...
  async authenticate(): Promise<void> {
    ...
    try {
      ...
      const xToken =
        this.configService.get<string>('SANKHYA_X_TOKEN') ||
        this.configService.get<string>('SANKHYA_TOKEN');
  ...
  async executeQuery<T = any>(sql: string): Promise<T[]> {
    ...
    try {
      ...
      if (response.data.status === '1') {
        ...
        const errorMsg =
          response.data.statusMessage || 'Erro na execução da query';
  ...
  async createAdjustmentNote(
    ...
    try {
      this.logger.log(
        `Criando Nota de Ajuste (TOP: ${top}, Itens: ${items.length})...`,
      );
      ...
      const requestBody = {
        nota: {
          cabecalho: {
            ...
            TIPMOV: {
              $:
                top === Number(this.configService.get('TOP_ENTRADA') || 221)
                  ? 'C'
                  : 'V',
            },
            ...(top === Number(this.configService.get('TOP_ENTRADA') || 221)
      ...
      if (response.data.status === '1') {
        ...
        const errorMsg =
          response.data.statusMessage || 'Erro na criação da nota';
  ...
  /**
   * Cria uma movimentação interna entre locais (TOP 700 / TIPMOV T).
   * O Sankhya representa transferência interna como par de itens:
   * sequência positiva baixa no local origem e sequência negativa entra no destino.
   */
  async createInternalTransferNote(
    codemp: number,
    dtneg: string,
    top: number,
    item: {
    ...
    observacao: string = 'Transferência interna gerada pelo App de Contagem',
  ): Promise<number> {
    await this.authenticate();

    try {
      this.logger.log(
        `Criando Movimentação Interna (TOP: ${top}, Produto: ${item.codprod}, Qtd: ${item.qtdneg})...`,
      );

      const controle = item.controle?.trim() || ' ';
      const valorUnitario = Number(item.vlrunit || 0);
      const requestBody = {
        nota: {
          cabecalho: {
            NUNOTA: {},
            ...
            CODNAT: {
              $: Number(
                this.configService.get('CODNAT_MOVIMENTACAO_INTERNA') ||
                  2010100,
              ),
            },
            CODCENCUS: {
              $: Number(
                this.configService.get('CODCENCUS_MOVIMENTACAO_INTERNA') ||
                  1040000,
              ),
            },
            OBSERVACAO: { $: observacao },
            ...
            item: [
              {
                ...
                CODPROD: { $: item.codprod },
                QTDNEG: { $: item.qtdneg },
                CODLOCALORIG: { $: item.codlocalOrigem },
                CODLOCALDEST: { $: item.codlocalDestino },
                VLRUNIT: { $: valorUnitario },
                CODVOL: { $: item.codvol || 'UN' },
                CONTROLE: { $: controle },
                CONTROLEDEST: { $: controle },
                PERCDESC: { $: 0 },
              },
            ],
          },
      ...
      if (response.data.status === '1') {
        ...
        const nunota = nunotaRaw['$'] ? nunotaRaw['$'] : nunotaRaw;
        this.logger.log(
          `✅ Movimentação interna criada com sucesso! NUNOTA: ${nunota}`,
        );
        return Number(nunota);
      }

      const errorMsg =
        response.data.statusMessage || 'Erro na criação da transferência';
      throw new Error(`Erro Sankhya TOP ${top}: ${errorMsg}`);
    } catch (error: any) {
      this.logger.error(`Sankhya Internal Transfer Error: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async findDailyAdjustmentNote(
    codemp: number,
    dtneg: string,
    top: number,
    observacaoPattern: string = '%App de Contagem%',
  ): Promise<number | null> {
    await this.authenticate();
    try {
  ...
  async addItemsToNote(
    ...
    try {
      ...
      if (response.data.status === '1') {
        ...
        throw new Error(
          `Erro Sankhya Add Items: ${response.data.statusMessage}`,
        );
      }
    } catch (error: any) {
  ...
  async confirmNote(nunota: number): Promise<void> {
    ...
    try {
      ...
      if (response.data.status === '1') {
        ...
        throw new Error(
          `Erro Sankhya CACSP.confirmarNota: ${response.data.statusMessage}`,
        );
      }
    } catch (error: any) {
  ...
  async getLatestSalePrice(codprod: number): Promise<number | null> {
    await this.authenticate();
    try {
      const sql = `
        SELECT EXC.VLRVENDA
        ...
        INNER JOIN TGFTAB TAB
          ON TAB.NUTAB = EXC.NUTAB
        WHERE EXC.CODPROD = ${codprod}
          AND TAB.DTVIGOR = (
            SELECT MAX(TAB2.DTVIGOR)
            FROM TGFEXC EXC2
            INNER JOIN TGFTAB TAB2
              ON TAB2.NUTAB = EXC2.NUTAB
            WHERE EXC2.CODPROD = EXC.CODPROD
              AND TAB2.DTVIGOR <= SYSDATE
          )
          AND ROWNUM = 1
      `;
      const result = await this.executeQuery<{ VLRVENDA: number }>(sql);
      if (result && result.length > 0) {
        return Number(result[0].VLRVENDA || 0) || null;
      }
      return null;
    } catch (error: any) {
      this.logger.warn(`Erro ao buscar preço de venda: ${error.message}`);
      return null;
    }
  }
    ...
    try {
      const requestBody = {
        dataSet: {
          ...
          dataRow: {
            localFields: Object.entries(fields).reduce(
              (acc, [key, value]) => {
                acc[key] = { $: value };
                return acc;
              },
      ...
      if (response.data.status === '1') {
        ...
        throw new Error(
          `Erro Sankhya Save Record (${entityName}): ${response.data.statusMessage}`,
        );
      }
    } catch (error: any) {
